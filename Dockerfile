...
# CoinGlass API documentation catalog — api/coinglass/catalog.py resolves it at
# <repo root>/CoinGlass API 介绍 (parents[3] of catalog.py = /app in the image);
# without it every proxied CoinGlass endpoint 404s as "not in the local catalog"
COPY ["CoinGlass API 介绍/", "./CoinGlass API 介绍/"]
